import { BarChart3, Truck, Fuel, Map, Zap, MapPin, Download, FileText } from 'lucide-react';
      ...
      <div className="flex items-center justify-start gap-2 sm:gap-4 py-2 px-2 sm:px-4 overflow-x-auto scrollbar-hide w-full">
          ...
          <button
            ...
            className={`flex flex-col items-center gap-2 px-4 sm:px-6 py-2 rounded-lg transition-all flex-shrink-0 ${
              isActive('/cargas') ? 'text-blue-400 bg-blue-500/10' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'
            }`}
          >
          ...
          {/* NF-e */}
          <button
            onClick={() => setLocation('/nfe')}
            className={`flex flex-col items-center gap-1 px-4 sm:px-6 py-1 rounded-lg transition-all flex-shrink-0 ${
              isActive('/nfe') ? 'text-blue-400 bg-blue-500/10' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'
            }`}
          >
            <FileText className="w-5 h-5" />
            <span className="text-xs font-medium whitespace-nowrap leading-tight">NF-e</span>
          </button>
